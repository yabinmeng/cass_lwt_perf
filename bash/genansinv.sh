...
pmsg "[nosqlbench]" $DSE_ANSINV_FILE
for ((i=0; i<${#dse_nodetypes[*]}; i++));
do
   if [[ ${dse_nodetypes[i]} == *"nb_clnt"* ]]; then
      pmsg "${public_ips[i]} private_ip=${private_ips[i]}" $DSE_ANSINV_FILE
   fi
done
pmsg "" $DSE_ANSINV_FILE
pmsg "" $DSE_ANSINV_FILE
...
do
   if [[ ${dse_nodetypes[i]} == *"opsc_srv"* ]]; then
      # only install OpsCenter server on one host, even if multiple hosts are specified/provisioned
...
cp $DSE_ANSINV_FILE ../ansible/hosts.ini
